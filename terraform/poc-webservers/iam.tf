...
resource "aws_iam_role" "webserver_role" {
  name               = "webserver-instance-role-${local.cluster_id}"
  assume_role_policy = data.aws_iam_policy_document.assume_role_policy.json
}

//TODO: Fine tune s3 permissions.  They've been added to allow ssm sesson manager logging.
data "aws_iam_policy_document" "webserver_cw_permissions" {
...
resource "aws_iam_role_policy" "webserver_policy" {
  name   = "webserver-instance-policy"
  policy = data.aws_iam_policy_document.webserver_cw_permissions.json
  role   = aws_iam_role.webserver_role.id
}

resource "aws_iam_instance_profile" "webserver_instance_profile" {
  name = aws_iam_role.webserver_role.name
  path = "/"
